...
- [ ] 所有机密存储在云机密管理器中（AWS Secrets Manager，Vercel Secrets）
- [ ] 启用数据库凭证的自动轮换
- [ ] API 密钥至少每季度轮换一次
- [ ] 代码、日志或错误消息中不包含机密
- [ ] 启用机密访问的审计日志
...
- [ ] **IAM**：未使用根账户，启用多因素认证，采用最小权限策略
- [ ] **密钥**：所有密钥存储在云密钥管理器中并进行轮换
- [ ] **网络**：安全组受限，数据库不对外公开
- [ ] **日志**：启用CloudWatch/日志记录并设置保留
- [ ] **监控**：已配置异常报警
- [ ] **CI/CD**：OIDC认证，密钥扫描，依赖审计
- [ ] **CDN/WAF**：启用Cloudflare WAF并配置OWASP规则
- [ ] **加密**：数据传输和静态均加密
- [ ] **备份**：自动备份并进行恢复测试
...
**记住**：云配置错误是数据泄露的主要原因。一个被公开的 S3 存储桶或权限过大的 IAM 策略都可能危及整个基础设施。始终遵循最小权限原则和纵深防御。
