...
Support multiple chat provider API protocols while preserving the existing Spring AI `ChatClient` integration used by business services.

## Requirements

### Requirement: Provider chat API type
The system SHALL allow every chat provider to declare one chat API type from `OPENAI_CHAT_COMPLETIONS`, `OPENAI_RESPONSES`, and `ANTHROPIC_MESSAGES`. Providers without a stored or configured API type SHALL be treated as `OPENAI_CHAT_COMPLETIONS`.
...
The system SHALL create the provider chat model according to the provider chat API type while continuing to expose Spring AI `ChatClient` to business services.
...
The system SHALL support synchronous text generation for all supported provider chat API types.
...
The system SHALL support streaming text generation for all supported provider chat API types.
...
The system SHALL test provider connectivity using the configured chat API type rather than always using the Chat Completions endpoint.
...
The system SHALL avoid silently enabling unsupported advanced features for provider chat API types whose adapters do not implement them.
